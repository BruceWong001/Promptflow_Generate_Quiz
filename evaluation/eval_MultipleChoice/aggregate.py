...
@tool
def aggregate(groundedness_scores: List[float],format_scores: List[float]):
    """
    ...
    for i in range(len(groundedness_scores)):
        ...
        aggregated_results["response_format"] = aggregated_results["response_format"] and format_scores[i]
    ...
    log_metric(key="response_format", value=aggregated_results["response_format"])
